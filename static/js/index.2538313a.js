(()=>{"use strict";var A,e,t,r,i={557(A,e,t){var r=t(848),i=t(540),a=t(338),n=t(983),o=t(651),l=t(106),s=t(274);let d=n.Ay.h1`
  font-weight: bold;
  font-size: 3rem;
  line-height: 5rem;
`,g=n.Ay.h3`
  text-decoration: underline dotted;
  font-weight: bolder;
`,c=n.Ay.img`
  width: 4rem;
  border-radius: 50px;
`,h=t.p+"static/image/wing-sprite.c198b9a2.png",B=[{text:"(I)ntro",path:"/"},{text:"(E)xperiences",path:"/exp"}],x=n.Ay.nav`
  margin: 1rem auto;
  width: fit-content;
  display: flex;
  align-items: center;
  gap: 0.5rem;
  @media (max-width: 425px) {
    flex-direction: row;
  }
`,m=(0,n.Ay)(o.k2)`
  text-decoration: none;
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  font-weight: bolder;
  font-size: 0.9rem;
  transition: color 0.2s ease-in-out;

  &.active {
    text-decoration: underline dotted;
  }

  &:hover {
    text-decoration: underline dotted;
  }
`,Q=n.Ay.span`
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  opacity: 0.3;
`,p=n.Ay.div``,E=n.Ay.div`
  background: url(${h}) no-repeat -3px -325px;
  background-size: 339px 366px;
  width: 58px;
  height: 41px;
  transform: scale(0.5);
  filter: ${A=>{let{theme:e}=A;return e.wingFilter}};
  transition: filter 0.5s ease-in-out;
  opacity: 0.3;

  @media (max-width: 425px) {
    display: none;
  }
`,u=n.Ay.div`
  background: url(${h}) no-repeat -3px -325px;
  background-size: 339px 366px;
  width: 58px;
  height: 41px;
  transform: scale(-0.5, 0.5);
  filter: ${A=>{let{theme:e}=A;return e.wingFilter}};
  transition: filter 0.5s ease-in-out;
  opacity: 0.3;

  @media (max-width: 425px) {
    display: none;
  }
`,I=()=>(0,r.jsxs)(x,{"data-testid":"navigation",children:[(0,r.jsx)(u,{"data-testid":"wing-left"}),B.map((A,e)=>{let{text:t,path:a}=A;return(0,r.jsxs)(i.Fragment,{children:[e>0&&(0,r.jsx)(Q,{children:"/"}),(0,r.jsx)(p,{children:(0,r.jsx)(m,{"data-testid":`nav-link-${a.replace("/","")||"home"}`,to:a,children:t})})]},e)}),(0,r.jsx)(E,{"data-testid":"wing-right"})]}),w=t.p+"static/image/portfolio-photo.d5f5c4c7.png",f={en:{greeting:"Hello world! This is ",description:", a software developer based in Taiwan. ",githubLabel:" on Github.",work:"- I currently work for ",company:"DBS Bank",workRole:" as a front-end developer.",skills:"- I write some JavaScript, CSS and HTML.",education:"I got my BS and MS degree in ",degree:"Computer Science",educationFrom:" from National Tsing Hua university."},zh:{greeting:"嗨！我是",description:"，來自台灣的軟體工程師。在 Github 上是 ",githubLabel:"",work:"- 目前在",company:"星展銀行",workRole:"擔任前端工程師。",skills:"- 主要撰寫 JavaScript、CSS 和 HTML。",education:"畢業於國立清華大學",degree:"資工系/資工所",educationFrom:""}},j=n.Ay.div`
  display: flex;
  gap: 1rem;
  @media (max-width: 768px) {
    flex-direction: column;
    align-items: center;
  }
`,C=n.Ay.img`
  width: 33%;
  height: auto;
  transition: background-color 0.3s ease-in-out, filter 0.3s ease-in-out;
  border-radius: 5px;
  background-color: ${A=>{let{theme:e}=A;return e.highlightBg}};
  cursor: pointer;
  filter: grayscale(100%);

  &:hover {
    background-color: ${A=>{let{theme:e}=A;return e.highlightBgHover}};
    filter: grayscale(0%);
  }

  @media (max-width: 768px) {
    display: none;
  }
`,M=n.Ay.div`
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  background-color: ${A=>{let{theme:e}=A;return e.highlightBg}};
  transition: background-color 0.3s ease-in-out;
  position: relative;

  border-radius: 5px;

  flex: 1;
  padding: 1rem;
  padding-bottom: 2rem;
  line-height: 1.5rem;
  font-size: 0.9rem;
  hyphens: auto;

  @media (max-width: 768px) {
    border-radius: 0;
  }
`,b=n.Ay.span`
  text-decoration: underline dotted;
  font-family: 'ndot-47', 'Lato-Regular', sans-serif;

  &:hover {
    text-decoration: none;
  }
`,D=n.Ay.button`
  font-family: 'ndot-47', 'Lato-Regular', sans-serif;
  background: transparent;
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  border: none;
  border-radius: 5px;
  cursor: pointer;
  font-size: 0.8rem;
  padding: 0.1rem 0.4rem;
  position: absolute;
  right: 0.5rem;
  bottom: 0.5rem;

  &:hover {
    text-decoration: underline dotted;
  }
`,S=()=>{let[A,e]=i.useState(w),[t,a]=i.useState(()=>localStorage.getItem("__lang")||"en"),n=f[t];return(0,r.jsxs)(j,{"data-testid":"intro",children:[(0,r.jsx)(C,{"data-testid":"profile-image",src:A,alt:"Portfolio image of Calvin Jeng"}),(0,r.jsxs)(M,{"data-testid":"intro-content",children:[n.greeting," ",(0,r.jsx)(b,{children:"Calvin Hao-Wei Jeng"}),n.description,(0,r.jsx)(b,{children:"@lockys"}),n.githubLabel,(0,r.jsx)("br",{}),(0,r.jsx)("br",{}),n.work,(0,r.jsx)(b,{children:n.company}),n.workRole," ",(0,r.jsx)("br",{}),n.skills," ",(0,r.jsx)("br",{}),(0,r.jsx)("br",{})," ",n.education,(0,r.jsx)(b,{children:n.degree}),n.educationFrom,(0,r.jsx)(D,{"data-testid":"lang-toggle",onClick:()=>{let A="en"===t?"zh":"en";a(A),localStorage.setItem("__lang",A)},children:"en"===t?"中":"EN"})]})]})},k=t.p+"static/image/nthu.a76a9325.jfif",y=n.Ay.div`
  display: flex;
  align-items: center;
  gap: 1rem;
  margin: 1rem 0;
`,F=n.Ay.div`
  font-size: 0.8rem;
  line-height: 1.2rem;
`,G=n.Ay.h4`
  font-weight: bolder;
`,U=n.Ay.div`
  @media (max-width: 768px) {
    margin: 0 1rem;
  }
`,v=(0,n.Ay)(g)`
  font-family: 'Lato-Regular', sans-serif;
  font-weight: normal;
`,z=()=>(0,r.jsxs)(U,{"data-testid":"exp",children:[(0,r.jsx)(v,{children:"(S)kills"}),(0,r.jsx)(y,{children:(0,r.jsxs)(F,{children:[(0,r.jsx)(G,{children:"Technical skills"}),(0,r.jsx)(F,{children:"JavaScript, React.js, CSS, HTML"}),(0,r.jsx)(G,{children:"Languages"}),(0,r.jsx)(F,{children:"Chinese(Native), English(Fair)"})]})}),(0,r.jsx)(v,{children:"(W)ork experiences"}),(0,r.jsxs)(y,{children:[(0,r.jsx)(c,{src:"data:image/jpeg;base64,"}),(0,r.jsxs)(F,{children:[(0,r.jsx)(G,{children:"DBS Bank."}),(0,r.jsxs)(F,{children:["Front-end developer, 2021/5 - PRESET, Taipei",(0,r.jsx)("br",{}),"Develop features for the Taiwan DBS internet banking website."]})]})]}),(0,r.jsxs)(y,{children:[(0,r.jsx)(c,{src:"data:image/jpeg;base64,"}),(0,r.jsxs)(F,{children:[(0,r.jsx)(G,{children:"Garmin corp."}),(0,r.jsxs)(F,{children:["Software engineer, 2016/11 - 2020/5, New Taipei",(0,r.jsx)("br",{}),"mostly focus on implementations of back-end system"]})]})]}),(0,r.jsx)(v,{children:"(E)ducation"}),(0,r.jsxs)(y,{children:[(0,r.jsx)(c,{src:k}),(0,r.jsxs)(F,{children:[(0,r.jsx)(G,{children:"National Tsing Hua university"}),(0,r.jsx)(F,{children:"MS, 2014 - 2016, Hsinchu"})]})]}),(0,r.jsxs)(y,{children:[(0,r.jsx)(c,{src:k}),(0,r.jsxs)(F,{children:[(0,r.jsx)(G,{children:"National Tsing Hua university"}),(0,r.jsx)(F,{children:"BS, 2010 - 2014, Hsinchu"})]})]})]}),J=n.Ay.div`
  text-align: center;
`,K=n.Ay.img`
  border-radius: 5px;
  width: 100%;
`,R=()=>(0,r.jsxs)(J,{"data-testid":"note",children:["Notes are still building. ",(0,r.jsx)("br",{}),(0,r.jsx)("br",{}),(0,r.jsx)(K,{"data-testid":"note-image",src:"https://media3.giphy.com/media/11ikeVaUfcXLWM/giphy.gif"})]}),O=(0,n.Ay)(g)`
  font-family: 'Lato-Regular', sans-serif;
  font-weight: normal;
`,H=()=>(0,r.jsxs)("div",{"data-testid":"kudos",style:{padding:"1rem"},children:[(0,r.jsx)(O,{children:"(K)udos"}),(0,r.jsx)("br",{}),(0,r.jsx)("section",{children:(0,r.jsxs)("ol",{children:[(0,r.jsxs)("li",{children:["- Fonts are from"," ",(0,r.jsx)("a",{target:"_blank",href:"https://fontstruct.com/fontstructions/show/2047444/ndot-45-inspired-by-nothing-1",rel:"noreferrer",children:"NDOT 47"})," ","and"," ",(0,r.jsx)("a",{target:"_blank",href:"https://larsenwork.com/monoid/",rel:"noreferrer",children:"Monoid"})]}),(0,r.jsxs)("li",{children:["- Wing image is from"," ",(0,r.jsx)("a",{target:"_blank",href:"https://www.shutterstock.com/",rel:"noreferrer",children:"shutterstock"})]}),(0,r.jsxs)("li",{children:["- Good old school gif is from"," ",(0,r.jsx)("a",{target:"_blank",href:"https://cyber.dabamos.de/88x31/index.html",rel:"noreferrer",children:"cyber.dabamos.de"})]})]})}),(0,r.jsx)("br",{}),(0,r.jsx)(O,{children:"(L)icense"}),(0,r.jsx)("br",{}),"This portfolio page is released under"," ",(0,r.jsx)("a",{href:"https://github.com/lockys/me/blob/master/license",target:"_blank",rel:"noreferrer",children:"MIT"}),"."]}),W=t.p+"static/svg/designed-by-human-badge.033b1cd2.svg",Y=["data:image/gif;base64,","data:image/gif;base64,","data:image/gif;base64,","data:image/gif;base64,","data:image/gif;base64,"],T=n.Ay.div`
  max-width: 768px;
  margin: 2rem auto;
  align-items: center;
  border-radius: 5px;
  color: ${A=>{let{theme:e}=A;return e.textColor}};
    
  @media (max-width: 768px) {
    margin: 0;
    border: none;
  }
`,L=(0,n.Ay)(d)`
  font-family: 'Lato-Regular', sans-serif;
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  text-align: right;
  padding: 0.5rem;
  font-size: 0.8rem;

  @media (max-width: 768px) {
    display: none;
  }
`,P=n.Ay.img`
  display: none;
  width: 60px;
  height: 60px;
  border-radius: 50%;
  object-fit: cover;
  filter: grayscale(100%);

  @media (max-width: 768px) {
    display: block;
    margin: 1rem auto 0;
  }
`,N=n.Ay.div`
  margin: 1rem;

  @media (max-width: 768px) {
    margin: 0;
  }
`,Z=n.Ay.section`
  margin: 1rem;

  @media (max-width: 768px) {
    margin: 0;
  }
`,V=n.Ay.footer`
  margin: 1rem;
  padding-top: 1rem;
  font-weight: bold;
`,q=n.Ay.a`
  padding: 0.2rem;
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  transition: color 0.3s ease-in-out;

  &:hover {
    color: ${A=>{let{theme:e}=A;return e.accentColor}};
  }
`,X=n.Ay.div`
  float: right;
  font-size: 0.8rem;

  @media (max-width: 320px) {
    float: none;
    padding: 1rem 0;
  }
`,$=n.Ay.section`
  font-family: 'ndot-47', 'Lato-Regular', sans-serif;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 0.9rem;
  margin: 1rem;
  gap: 0.3rem;

  @media (max-width: 320px) {
    flex-direction: column;
  }
`,_=n.Ay.span`
  color: ${A=>{let{theme:e}=A;return e.accentColor}};
`,AA=n.Ay.hr`
  line-height: 1em;
  position: relative;
  outline: 0;
  border: 0;
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  text-align: center;
  height: 1.5em;
  opacity: 0.5;

  &:before {
    content: '';
    background: linear-gradient(to right, transparent, ${A=>{let{theme:e}=A;return e.textColor}}, transparent);
    position: absolute;
    left: 0;
    top: 50%;
    width: 100%;
    height: 1px;
  }

  &:after {
    content: attr(data-content);
    position: relative;
    display: inline-block;
    padding: 0 0.5em;
    line-height: 1.5em;
    color: ${A=>{let{theme:e}=A;return e.textColor}};
    background-color: ${A=>{let{theme:e}=A;return e.background}};
    transition: background 0.5s ease-in-out;
  }
`,Ae=(0,n.i7)`
  from {
    height: 0;
  }

  to {
    height: auto;
  }
`,At=n.Ay.div`
  height: 0;
  overflow: hidden;
  color: ${A=>{let{theme:e}=A;return e.accentColor}};
  animation: ${Ae} 1s linear infinite;

  @media (max-width: 320px) {
    display: none;
  }
`,Ar=(0,n.Ay)(o.N_)`
  color: ${A=>{let{theme:e}=A;return e.textColor}};
  text-decoration: none;
`,Ai=n.Ay.div`
  cursor: pointer;
  display: inline-block;
  margin-left: 1rem;
  vertical-align: middle;

  &:hover {
    text-decoration: underline dotted;
  }
`,Aa=n.Ay.div`
  display: flex;
  justify-content: end;
  margin: 1rem;
`,An=n.Ay.img`
  border: 1px solid ${A=>{let{theme:e}=A;return e.border}};
  cursor: pointer;
`,Ao=n.Ay.img`
  margin-left: 0.5rem;
  height: 31px;
  filter: ${A=>{let{theme:e}=A;return e.wingFilter}};
  transition: filter 0.5s ease-in-out;
`,Al=function(A){let e,t,{setIsDark:a,isDark:n}=A,[o,d]=i.useState(Y[0]);return(0,r.jsxs)(T,{"data-testid":"portfolio-container",children:[(0,r.jsxs)(N,{"data-testid":"header",children:[(0,r.jsx)(L,{"data-testid":"header-title",children:"I dont know what to put here yet."}),(0,r.jsx)(P,{"data-testid":"mobile-avatar",src:w,alt:"Calvin Jeng"})]}),(0,r.jsx)(I,{}),(0,r.jsx)(Z,{"data-testid":"content-section",children:(0,r.jsxs)(l.BV,{children:[(0,r.jsx)(l.qh,{exact:!0,path:"/",element:(0,r.jsx)(S,{})}),(0,r.jsx)(l.qh,{path:"/exp",element:(0,r.jsx)(z,{})}),(0,r.jsx)(l.qh,{path:"/note",element:(0,r.jsx)(R,{})}),(0,r.jsx)(l.qh,{path:"/kudos",element:(0,r.jsx)(H,{})})]})}),(0,r.jsxs)($,{"data-testid":"time-section",children:["\uD83D\uDD51 Taipei Time : UTC+8 = ",(0,r.jsx)(_,{"data-testid":"time-value",children:(t=new Date((e=new Date).getTime()+6e4*e.getTimezoneOffset()+288e5),`${t.getHours()}:${t.getMinutes().toLocaleString("en-US",{minimumIntegerDigits:2,useGrouping:!1})}`)}),(0,r.jsx)(At,{children:"."})]}),(0,r.jsxs)(Aa,{"data-testid":"reaction-section",children:[(0,r.jsx)(An,{"data-testid":"random-gif",src:o,onClick:()=>d(Y[Math.floor(Math.random()*Y.length)]),alt:"gif"}),(0,r.jsx)("a",{href:"https://notbyai.fyi",target:"_blank",rel:"noreferrer",children:(0,r.jsx)(Ao,{"data-testid":"not-by-ai-badge",src:W,alt:"Designed by Human, Not by AI"})})]}),(0,r.jsx)(AA,{"data-testid":"site-divider",onClick:()=>{},className:"hr-text","data-content":"https://calvinjeng.io"}),(0,r.jsxs)(V,{"data-testid":"footer",children:[(0,r.jsx)(q,{"data-testid":"link-github",href:"https://github.com/lockys",target:"_blank",children:(0,r.jsx)(s.BR8,{})}),(0,r.jsx)(q,{"data-testid":"link-linkedin",href:"https://www.linkedin.com/in/calvinjeng/",target:"_blank",children:(0,r.jsx)(s.Wjy,{})}),(0,r.jsx)(q,{"data-testid":"link-email",href:"mailto:lockys.tw@gmail.com",target:"_blank",children:(0,r.jsx)(s.pHD,{})}),(0,r.jsxs)(X,{"data-testid":"site-info",children:[(0,r.jsx)(Ar,{to:"/kudos",children:"\xa9"})," ",new Date().getFullYear()," Calvin Hao-Wei Jeng",(0,r.jsx)(Ai,{"data-testid":"theme-toggle",onClick:()=>{let A=!n;a(A),localStorage.setItem("__isDark",String(A))},children:n?"明":"暗"})]})]})]})};var As=t(716);let Ad={textColor:"#2A4D4C",background:"#fffbf8",wingFilter:"none",border:"#eee",containerBorder:"#A2AAAD",accentColor:"#c7372f",highlightBg:"#E2FFF2",highlightBgHover:"#F0FFF7"},Ag={textColor:"#e0e0e0",background:"#1a1a2e",wingFilter:"invert(0.85)",border:"#444",containerBorder:"#555",accentColor:"#ff6b6b",highlightBg:"#1e3a2f",highlightBgHover:"#254a3a"},Ac=(0,n.DU)`
  ${As.Ay}
  html {
    font-family: 'Lato-Regular', sans-serif;
    transition: background .5s ease-in-out;
    background: ${A=>{let{theme:e}=A;return e.background}};
  }

  a {
    color: ${A=>{let{theme:e}=A;return e.textColor}};
  }
`;a.createRoot(document.getElementById("root")).render((0,r.jsx)(()=>{let[A,e]=i.useState("true"===localStorage.getItem("__isDark"));return(0,r.jsx)(i.StrictMode,{children:(0,r.jsxs)(n.NP,{theme:A?Ag:Ad,children:[(0,r.jsx)(Ac,{}),(0,r.jsx)(o.Kd,{children:(0,r.jsx)(Al,{setIsDark:e,isDark:A})})]})})},{}))}},a={};function n(A){var e=a[A];if(void 0!==e)return e.exports;var t=a[A]={exports:{}};return i[A](t,t.exports,n),t.exports}n.m=i,n.n=A=>{var e=A&&A.__esModule?()=>A.default:()=>A;return n.d(e,{a:e}),e},n.d=(A,e)=>{for(var t in e)n.o(e,t)&&!n.o(A,t)&&Object.defineProperty(A,t,{enumerable:!0,get:e[t]})},n.o=(A,e)=>Object.prototype.hasOwnProperty.call(A,e),A=[],n.O=(e,t,r,i)=>{if(t){i=i||0;for(var a=A.length;a>0&&A[a-1][2]>i;a--)A[a]=A[a-1];A[a]=[t,r,i];return}for(var o=1/0,a=0;a<A.length;a++){for(var[t,r,i]=A[a],l=!0,s=0;s<t.length;s++)(!1&i||o>=i)&&Object.keys(n.O).every(A=>n.O[A](t[s]))?t.splice(s--,1):(l=!1,i<o&&(o=i));if(l){A.splice(a--,1);var d=r();void 0!==d&&(e=d)}}return e},n.p="/",e={410:0},n.O.j=A=>0===e[A],t=(A,t)=>{var r,i,[a,o,l]=t,s=0;if(a.some(A=>0!==e[A])){for(r in o)n.o(o,r)&&(n.m[r]=o[r]);if(l)var d=l(n)}for(A&&A(t);s<a.length;s++)i=a[s],n.o(e,i)&&e[i]&&e[i][0](),e[i]=0;return n.O(d)},(r=self.webpackChunkcalvinjeng=self.webpackChunkcalvinjeng||[]).forEach(t.bind(null,0)),r.push=t.bind(null,r.push.bind(r));var o=n.O(void 0,["783","535","625"],()=>n(557));o=n.O(o)})();